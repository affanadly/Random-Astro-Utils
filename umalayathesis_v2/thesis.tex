...
\gls{ALMA} and \gls{VLBI} are two of the most powerful tools in modern astronomy. \gls{ALMA} is an interferometer located in the Atacama Desert in Chile, which operates at millimeter and submillimeter wavelengths. It consists of 66 high-precision antennas that can be arranged in different configurations to achieve high angular resolution and sensitivity. \gls{VLBI} is a technique that combines signals from multiple radio telescopes around the world to create a virtual telescope with an effective diameter equal to the maximum separation between the telescopes, allowing for extremely high angular resolution, \gls{theta}.
...
Tables can also be added as usual using the \texttt{table} and \texttt{tabular} environments. For more advanced customization, the \texttt{tabularx} environment can be used (assuming that it has been loaded into the preamble). The following is an example command for the \texttt{table} and \texttt{tabular} environment, followed by the output in Table \ref{tab:table}:
