...
@proceedings{EAVN2024,
  title = {{VLBI} Observations of 6.7 {GHz} methanol masers during maser flares},
  author = {Ross, A. Burns},
  year = {2024},
  month = dec,
  booktitle = {{East Asian VLBI Network 2024}},
...
\newacronym{theta}{$\theta_D$}{Diffraction limit}
...
\begin{minted}{latex}
    ...
    \usepackage{amsmath} % complex equation environments
\end{minted}
...
Write the English and Bahasa Malaysia abstracts in the \texttt{abstract} and \texttt{abstrak} environments respectively. If \mintinline{latex}|\tajuk| is defined, the Bahasa Malaysia abstract title will be prepended with that. A maximum of five (5) keywords should be listed below the abstract within their respective environments with the following format:
...
The main chapters of the thesis should start with the \mintinline{latex}|\mainmatter| command. Then, the usual commands i.e. \mintinline{latex}|\chapter|, \mintinline{latex}|\section|, \mintinline{latex}|\subsection|, \mintinline{latex}|\subsubsection| can be used. Fifth level headings are also possible using \mintinline{latex}|\subsubsubsection|, but it will not appear in the Table of Contents (and not recommended in general). It is recommended that each chapter/section is split into different \texttt{tex} files and loaded into the main document file using the \mintinline{latex}|\input| command.
...
Note that landscape figures can be enclosed in a \verb|\afterpage| command from the \texttt{afterpage} package to ensure that the figure does not disrupt the document flow.
...
Similar to landscape figures, landscape tables can also be enclosed in a \verb|\afterpage| command to ensure that the table does not disrupt the document flow. If a table is too long to fit in a single page, the table should be created with a \texttt{longtable} environment instead (assuming that it has been loaded into the preamble). The following is an example command for the \texttt{longtable} environment, and it is rendered in Appendix \ref{app:longtable}:
\begin{minted}{latex}
    \begin{longtable}{ccc}
...
\end{equation}
Note that to call a reference to the equation, the label command i.e. \mintinline{latex}{\label{...}} should be set. In this case, \mintinline{latex}{Equation \ref{eq:energy}} would return -- Equation \ref{eq:energy}. To remove the numbering for the entire environment, use the starred version i.e. \mintinline{latex}{\begin{equation*}...\end{equation*}}. For equations with multiple lines (without any necessary alignment), use the \texttt{gather} environment, and similarly, to remove the numbering for the entire environment, use the starred version. 
...
The \texttt{glossaries} package is pre-loaded into the class to manage symbols and abbreviations. To define a symbol or an abbreviation, use the \mintinline{latex}|\newacronym| command with the following syntax:
\mint{latex}|\newacronym{label}{abbrv}{long}|
where \texttt{label} is the unique identifier for the acronym, \texttt{abbrv} is the abbreviation, and \texttt{long} is the full form. To call symbols, only use the \mintinline{latex}|\acs{label}| command. To call abbreviations, use \mintinline{latex}|\ac{label}|, which will print the long form followed by the abbreviation in parentheses on the first use, and only the abbreviation for subsequent uses. For example:
\begin{quote}
    \ac{VLBI} is a powerful technique in radio astronomy. \ac{VLBI} allows for extremely high angular resolution, \acs{theta}. Examples of \ac{VLBI} arrays include the Event Horizon Telescope and \ac{ALMA}. \ac{ALMA} is an interferometer located in the Atacama Desert in Chile, which operates at millimeter and submillimeter wavelengths.
\end{quote}

For plural forms, use \mintinline{latex}|\acp|. To capitalise the first letter, use \mintinline{latex}|\Ac|. To force the short, long, or full form, use \mintinline{latex}|\acs|, \mintinline{latex}|\acl|, and \mintinline{latex}|\acf|, respectively (independent of first use). For more advanced usage, the abbreviation can be defined with \mintinline{latex}|\newglossaryentry| command, with the following syntax:
\begin{minted}{latex}
    \newglossaryentry{label}{
        ...
        first={first long},
        plural={abbrvs},
        firstplural={first long plural}
    }
\end{minted}
where \texttt{first}, \texttt{plural}, and \texttt{firstplural} are the custom formats for the first use of the abbreviation in singular, the plural, and the first use in plural forms, respectively.
...
where \texttt{key} corresponds to the entry keys in the bibliography file(s), \texttt{prenote} is printed at the beginning of the citation, and \texttt{postnote} is printed at the end of citation. If the bibliographic entry begins with a lowercase, the citation command can be forced to uppercase using \mintinline{latex}|\Command| i.e. if the citation begins a sentence (this is overriden if the command has a prenote). Multiple citations can be done by listing all the keys, but unique pre- and/or postnotes for each key can be invoked using \mintinline{latex}|\commands| with the following syntax:
...
\mintinline{latex}|\parencite| This creates the usual APA style 
citation in parentheses: \parencite{Rosli2023}. 
\begin{itemize}[nosep]
