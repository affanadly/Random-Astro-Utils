...
To call symbols, only use the \mintinline{latex}|\acs{label}| command. To call the abbreviation, use \mintinline{latex}|\ac{label}|, which will print the long form followed by the abbreviation in parentheses on the first use, and only the abbreviation for subsequent uses. For example:
...
For plural forms, use \mintinline{latex}|\acp{label}|. To capitalise the first letter, use \mintinline{latex}|\Ac{label}|. To force the short, long, or full form, use \mintinline{latex}|\acs{label}|, \mintinline{latex}|\acl{label}|, and \mintinline{latex}|\acf{label}|, respectively (independent of first use). For more advanced usage, 
