...
where \texttt{label} is the unique identifier for the acronym, \texttt{abbrv} is the abbreviation, and \texttt{long} is the full form. To call symbols, only use the \mintinline{latex}|\acs{label}| command. To call the abbreviation, use \mintinline{latex}|\ac{label}|, which will print the long form followed by the abbreviation in parentheses on the first use, and only the abbreviation for subsequent uses. For example:
...
For plural forms, use \mintinline{latex}|\acp{label}|. To capitalise the first letter, use \mintinline{latex}|\Ac{label}|. To force the short, long, or full form, use \mintinline{latex}|\acs{label}|, \mintinline{latex}|\acl{label}|, and \mintinline{latex}|\acf{label}|, respectively (independent of first use). For more advanced usage, the abbreviation can be defined with \mintinline{latex}|\newglossaryentry| command, with the following syntax:
...
where \texttt{first} and \texttt{firstplural} are the formats for the first use of the abbreviation in singular and plural forms, respectively, and this is usually used for defininng proper nouns prepended by a definite article.
